...
function shouldIgnore(path, source) {
  ...
  const ignore = getAnnotation(source, 'compile') === 'false' ||
            ...
            getAnnotation(source, 'lessbuild') === 'false';   // 兼容原来的配置
...
function getTpl(settings) {
  const assets = settings.assets || {};
  const prefix = assets.prefix || '/g';

  const defTpl = prefix + '/{name}/{path}';
  const tpl = settings.development ? defTpl :
      assets.urlPattern || defTpl;

  let render = tplCache.get(tpl);
  if (!render) {
    render = template(tpl, { interpolate: /\{([\s\S]+?)\}/g });
    tplCache.set(tpl, render);
  }
  return render;
}
